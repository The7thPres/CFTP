...
//  input  EXC_SReset,              // Soft Reset (not implemented)
    ...
    output reg [31:0] Exc_PC_Out,   // Address for PC at the beginning of an exception
	 ...
	 output reg vote_Status_CU_0,
	 output reg vote_Status_RE,
	 output reg [7:0] vote_Status_IM,
	 output reg vote_Status_UM,
	 output reg vote_Status_IE,
	 output reg vote_Cause_IV,
	 output reg [7:0] vote_Cause_IP,
	 output reg vote_Cause_BD,
	 output reg [1:0] vote_Cause_CE,
	 output reg [3:0] vote_Cause_ExcCode30,
	 output reg vote_Status_EXL,
    ...
    */
    ...
    //   Cause_IV;                  // Indicator of general IV (0->0x180) or special IV (1->0x200)
    wire Cause_WP = 0;
    //   Cause_IP;          		   // Pending HW Interrupt indicator.
    wire Cause_ExcCode4 = 0;        // Can be made into a register when this bit is needed.
    //   Cause_ExcCode30;     		// Description of Exception (only lower 4 bits currently used; see above)
    ...
    */
    ...
    always @(posedge clock) begin
        if (reset) begin
            ...
            vote_Status_BEV    <= (CP0_WriteCond & (Rd == 5'd12) & (Sel == 3'b000)) ? Reg_In[22]   : Status_BEV;
            vote_Status_NMI    <= (CP0_WriteCond & (Rd == 5'd12) & (Sel == 3'b000)) ? Reg_In[19]   : Status_NMI;
            vote_Status_ERL    <= (CP0_WriteCond & (Rd == 5'd12) & (Sel == 3'b000)) ? Reg_In[2]    : ((Status_ERL & ERET & ~ID_Stall) ? 1'b0 : Status_ERL);
            vote_ErrorEPC      <= (CP0_WriteCond & (Rd == 5'd30) & (Sel == 3'b000)) ? Reg_In       : ErrorEPC;
        end
    end
    ...
    always @(posedge clock) begin
        if (reset) begin
            ...
            vote_Count         <= (CP0_WriteCond & (Rd == 5'd9 ) & (Sel == 3'b000)) ? Reg_In       : Count + 1;
            vote_Compare       <= (CP0_WriteCond & (Rd == 5'd11) & (Sel == 3'b000)) ? Reg_In       : Compare;
            vote_Status_CU_0   <= (CP0_WriteCond & (Rd == 5'd12) & (Sel == 3'b000)) ? Reg_In[28]   : Status_CU_0;
            vote_Status_RE     <= (CP0_WriteCond & (Rd == 5'd12) & (Sel == 3'b000)) ? Reg_In[25]   : Status_RE;
            vote_Status_IM     <= (CP0_WriteCond & (Rd == 5'd12) & (Sel == 3'b000)) ? Reg_In[15:8] : Status_IM;
            vote_Status_UM     <= (CP0_WriteCond & (Rd == 5'd12) & (Sel == 3'b000)) ? Reg_In[4]    : Status_UM;
            vote_Status_IE     <= (CP0_WriteCond & (Rd == 5'd12) & (Sel == 3'b000)) ? Reg_In[0]    : Status_IE;
            vote_Cause_IV      <= (CP0_WriteCond & (Rd == 5'd13) & (Sel == 3'b000)) ? Reg_In[23]   : Cause_IV;
             ...
             */
            vote_Cause_IP[7]   <= (CP0_WriteCond & (Rd == 5'd11) & (Sel == 3'b000)) ? 1'b0 : ((Int5) ? 1'b1 : Cause_IP[7]);
            vote_Cause_IP[6:2] <= Int[4:0];
            vote_Cause_IP[1:0] <= (CP0_WriteCond & (Rd == 5'd13) & (Sel == 3'b000)) ? Reg_In[9:8]  : Cause_IP[1:0];
        end
    end
    ...
    always @(posedge clock) begin
        if (reset) begin
            ...
            if (M_Exception_Ready) begin
                ...
                vote_Cause_CE <= (COP3) ? 2'b11 : ((COP2) ? 2'b10 : ((COP1) ? 2'b01 : 2'b00));
            ...
            // No exceptions this cycle
                ...
                vote_Status_EXL <= (CP0_WriteCond & (Rd == 5'd12) & (Sel == 3'b000)) ? Reg_In[1] : ((Status_EXL & ERET & ~ID_Stall) ? 1'b0 : Status_EXL);
                // The EPC is also writable by software
                vote_EPC <= (CP0_WriteCond & (Rd == 5'd14) & (Sel == 3'b000)) ? Reg_In : EPC;
                vote_BadVAddr <= BadVAddr;
            end
        end
    end
